...
def cal_performance(pred, gold, smoothing=False):
    ...
    pred = pred.max(1)[1]  # max idx for each row,  n
    gold = gold.contiguous().view(-1)  # n
    non_pad_mask = gold.ne(Constant.PAD)
    n_correct = pred.eq(gold)  # Byte vector
    n_correct = n_correct.masked_select(non_pad_mask).sum().item()  # number
...
def train_epoch(model, training_data, optimizer, device, smoothing):
    ...
    model.train()  # Open the train model
