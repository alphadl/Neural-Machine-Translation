...
# preprocess.py 2019-06-20 14:57
'''Handle the data io'''
...
def read_instance_from_file(inst_file, max_len, keep_case):
    '''convert file into word seq lists and vocab'''
    word_insts = []  # 词汇化后的句子集合
    trimmed_sent_count = 0  # 因为句长限制 被修剪的句子个数
    ...
    if trimmed_sent_count > 0:
        print('[Warning] {} instances are trimmed to the max sentence length {}.'.format(trimmed_sent_count, max_len))
...
def main():
    ...
    # Training set
    train_src_word_insts = read_instance_from_file(
        opt.train_src, opt.max_len, opt.keep_case
    )

    train_tgt_word_insts = read_instance_from_file(
    ...
    # validation set
    valid_src_word_insts = read_instance_from_file(
        opt.valid_src, opt.max_len, opt.keep_case
    )
    valid_tgt_word_insts = read_instance_from_file(
