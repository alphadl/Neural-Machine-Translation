...
class Beam():
    ...
    def advance(self, word_prob):
        ...
        # best score id is flattened as a (beam x word) array
        # we need to calculate which word and beam each score came from
        prev_k = best_scores_id / num_words
        self.prev_ks.append(prev_k)
        self.next_ys.append(best_scores_id - prev_k * num_words)

        # End condition is when top-of-beam is EOS.
        if self.next_ys[-1][0].item() == Constant.EOS:
            self._done = True
            self.all_scores.append(self.scores)
    ...
    def get_the_best_score_and_idx(self):
        ...
        scores, idx = self.sort_scores()
        return scores[1], idx[1]
    ...
    def get_tentative_hypothesis(self):
        ...
        if len(self.next_ys) == 1:
            ...
            hyps = [[Constant.BOS] + h for h in hyps]
