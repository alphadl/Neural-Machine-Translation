...
def get_attn_key_pad_mask(seq_k, seq_q):
    '''For masing out the padding part of key sequence'''
...
class Encoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 n_layers, n_head, d_model, d_inner, dropout=0.1
                 ):
        ...
        self.src_word_emb = nn.Embedding(
            n_src_vocab, d_word_vec, padding_idx=Constant.PAD
        )
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.src_word_emb = nn.Embedding(
            n_tgt_vocab, d_word_vec, padding_idx=Constant.PAD
        )
    ...
    def forward(self, tgt_seq, tgt_pos, src_seq, enc_output, return_attns=False):
        ...
        if return_attns:
            return enc_output, dec_slf_attn_list, dec_enc_attn_list
...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        if tgt_emb_prj_weight_sharing:
            # weight sharing (aka.weight tying)
            # between target word embedding & final logit dense layer.
            # see : https://arxiv.org/pdf/1608.05859.pdf
            self.tgt_word_prj.weight = self.decoder.tgt_word_emb.weight
            self.x_logit_scale = (d_model ** -0.5)  # scaling trick
        else:
            self.x_logit_scale = 1.  # no scaling trick

        if emb_src_tgt_weight_sharing:
            # weight sharing (aka. weight tying)
            # share the weight matrix between src&tgt word embeddings.
            # see: https://arxiv.org/pdf/1608.05859.pdf
            assert n_src_vocab == n_tgt_vocab  # ensure same vocab size of src/ tgt
