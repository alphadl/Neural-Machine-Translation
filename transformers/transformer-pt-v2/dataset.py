...
def collate_fn(insts):
    ...
    # Padding in the end of sequence
    batch_seq = np.array([
        inst + [Constant.PAD] * (max_len - len(inst))
        for inst in insts
    ])

    # Keep the position number except the padding symbols
    batch_pos = np.array([
        [pos_i if w_i != Constant.PAD else 0 for pos_i, w_i in enumerate(inst)] for inst in batch_seq
    ])
